...
from async_task_runner.src.exceptions import InvalidRetryLimitException
from async_task_runner.src.exceptions import InvalidRetryTimeoutException
                ...
                if retry_failures and retries < retry_limit:
                    ...
                    new_tasks[task.get_name()] = (
                        asyncio.ensure_future(
                            execute_with_semaphore(new_tasks[task.get_name()][1]())
                        ),
                    ...
                    logger.info(
                        f"{new_tasks[task.get_name()][1]} scheduled for restart."
                    )
